                ...
                <a-spin :spinning="spinning">
                    ...
                    API + '/fichier/photo_de_salle/' + record.image_salle" style="width: 60px;height: 60px;border-radius: 100px;cursor: pointer;" />
                        ...
                        <template #titre_residence="{ record }">
                            ...
                            <div>
                                Tarif : <a-button type="primary" shape="round" size="small"><strong> {{formatteurMillier (record.prix_salle)}} </strong></a-button>
                            </div>
                        </template>
                        ...
                        <template #action="{record}">
                            <a-space>
                                ...
                                <template v-if="record.etat_reservation == 1">
...
import {
    ...
    EP_LISTE_RESERVATION_HOTEL_CHAMBRE,
    EP_LIBERER_RESERVATION_HOTEL,
    EP_ANNULER_RESERVATION_HOTEL
...
export default {
    ...
    methods: {
        listeHotelCHambre() {
            ...
            processDataAuth(EP_LISTE_RESERVATION_HOTEL_CHAMBRE, data).then((res) => {
                this.spinning = !this.spinning;
                this.demandes = res.resultat;
            })
        },
        handleSearch() {
            this.listeHotelCHambre();
        },
        ...
        handleActionLiberer(record) {
            ...
            processDataAuth(EP_LIBERER_RESERVATION_HOTEL, data).then((res) => {
                this.spinning = !this.spinning;
        ...
        handleActionAnnuler() {
            ...
            processDataAuth(EP_ANNULER_RESERVATION_HOTEL, data).then((res) => {
                this.spinning = !this.spinning;
                if (res.status == 'succes') {
                    notification.success({
                        message: res.message
