                ...
                <a-spin :spinning="searchLoad">
                    ...
                    API + '/fichier/photo_de_bien/' + record.image_residence" style="width: 60px;height: 60px;border-radius: 100px;cursor: pointer;" />
                        ...
                        <template #tarif={record}>
                            <a-button type="primary" shape="" size="small"><strong> {{formatteurMillier (record.tarif)}} </strong></a-button>
                        </template>
...
export default {
    ...
    methods: {
        listeDemandeResidence() {
            ...
            processDataAuth(EP_LISTE_DEMANDE_RESIDENCE, data).then((res) => {
                this.searchLoad = false;
                this.demandes = res.resultat;
            })
        },
        handleSearch() {
            this.listeDemandeResidence();
        },
