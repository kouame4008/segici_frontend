...
<ul class="nav nav-tabs" id="myTab" role="tablist">
    <li class="nav-item">
        <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">BIEN EN LOCATION</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false">BIEN EN VENTE</a>
    </li>
</ul>
...
<div class="tab-content">
    <div class="tab-pane active" id="home" role="tabpanel" aria-labelledby="home-tab">
        <div class="mt-3">
            <div class="row">
                <template v-for="(el, i) in bienEnLocation" :key="i">
                    <avantage :avantage="el.titre_avantage" :descrip="el.description_avantage" />
                </template>
            </div>

            <div class="mt-4 row">
                <a-button type="" class="a-button" size="large" @click="changeVisible ('BIENS EN LOCATION',1) " style="margin-left: 20px;">
                    DEVENIR PARTENAIRE
    ...
    <div class="tab-pane" id="profile" role="tabpanel" aria-labelledby="profile-tab">
        <div class="mt-3">
            <div class="row">
                <template v-for="(el, i) in bienEnVente" :key="i">
                    <avantage :avantage="el.titre_avantage" :descrip="el.description_avantage" />
                </template>
            </div>

            <div class="mt-4 row">
                <a-button type="" class="a-button" size="large" @click="changeVisible ('BIENS EN VENTE',1) " style="margin-left: 20px;">
                    DEVENIR PARTENAIRE
                    <ArrowRightOutlined style="position : relative; top : -4px" />
                </a-button>
            </div>
...
export default {
    ...
    setup() {
        ...
        const handleBienEnLocation = () => {
            ...
            data.append('type_avantage_id', 1)
            processData(LISTE_AVANTAGE, data).then((res) => {
                bienEnLocation.value = res.resultat;
            })

            // Bien en vente
            let dataBV = new FormData();
            dataBV.append('type_avantage_id', 2)
            processData(LISTE_AVANTAGE, dataBV).then((res) => {
                bienEnVente.value = res.resultat;
            })
